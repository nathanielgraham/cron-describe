...
describe('CronParser', () => {
  ...
  const from = DateTime.fromISO('2025-09-28T12:00:00', { zone: timeZone });
  test('regular cron: every hour at :00:00', () => {
    const parser = new CronParser('0 * * * *', timeZone);
  ...
  test('quartz: every day at 00:00:00', () => {
    const parser = new CronParser('0 0 0 * * ?', timeZone);
  ...
  test('last day of month at 00:00:00', () => {
    const parser = new CronParser('0 0 0 L * ?', timeZone);
  ...
  test('last Friday of month at 00:00:00', () => {
    const parser = new CronParser('0 0 0 ? * 5L', timeZone);
  ...
  test('second Tuesday of month at 00:00:00', () => {
    const parser = new CronParser('0 0 0 ? * 2#2', timeZone);
  ...
  test('nearest weekday to 26th at 00:00:00', () => {
    const parser = new CronParser('0 0 0 26W * ?', timeZone);
  ...
  test('last weekday of month at 00:00:00', () => {
    const parser = new CronParser('0 0 0 LW * ?', timeZone);
    expect(parser.next(from)).toBe(1759190400); // 2025-09-30T00:00:00Z
  ...
  test('every 15 minutes', () => {
    const parser = new CronParser('0 */15 * * * ?', timeZone);
  ...
  test('every Monday in January', () => {
    const parser = new CronParser('0 0 0 ? JAN MON', timeZone);
  ...
  test('invalid cron expression', () => {
    expect(() => new CronParser('0 0 0 1 * 1,2', timeZone)).toThrow('Both day-of-month and day-of-week cannot be specified unless one is * or ?');
  });
  test('every 5 seconds', () => {
    const parser = new CronParser('*/5 * * * * ?', timeZone);
  ...
  test('specific days of the month: 1st and 15th at 12:00:00', () => {
    const parser = new CronParser('0 0 12 1,15 * ?', timeZone);
  ...
  test('third Wednesday in March and April', () => {
    const parser = new CronParser('0 0 0 ? MAR,APR WED#3', timeZone);
  ...
  test('common: daily at 08:30:00', () => {
    const parser = new CronParser('0 30 8 * * ?', timeZone);
  ...
  test('common: weekly on Wednesday at 14:00:00', () => {
    const parser = new CronParser('0 0 14 ? * WED', timeZone);
  ...
  test('common: yearly on February 29th at 00:00:00 (leap year)', () => {
    const parser = new CronParser('0 0 0 29 FEB ?', timeZone);
  ...
  test('obscure: last second of every minute', () => {
    const parser = new CronParser('59 * * * * ?', timeZone);
  ...
  test('obscure: only in 2025 on April 1st at 12:00:00', () => {
    const parser = new CronParser('0 0 12 1 APR ? 2025', timeZone);
  ...
  test('obscure: every even minute on odd days in even months', () => {
    const parser = new CronParser('0 */2 * 1-31/2 */2 ?', timeZone);
    expect(parser.next(from)).toBe(1759320000); // 2025-10-01T12:00:00Z
  ...
  test('edge: invalid step value', () => {
    expect(() => new CronParser('0 */0 * * * ?', timeZone)).toThrow('Invalid step value');
  });
  ...
  test('edge: maximum values (59 59 23 31 12 ? 2099)', () => {
    const parser = new CronParser('59 59 23 31 12 ? 2099', timeZone);
  ...
  test('edge: invalid month value', () => {
    expect(() => new CronParser('0 0 0 * 13 ?', timeZone)).toThrow('Invalid value');
  });
  test('edge: invalid day-of-week value', () => {
    expect(() => new CronParser('0 0 0 ? * 8', timeZone)).toThrow('Invalid value');
  });
  test('edge: invalid nth value for day-of-week', () => {
    expect(() => new CronParser('0 0 0 ? * MON#6', timeZone)).toThrow('Invalid nth value for day of week');
  });
  test('complex: specific minutes with step interval', () => {
    const parser = new CronParser('0 0,30/15 * * * ?', timeZone);
    expect(parser.next(from)).toBe(1759061700); // 2025-09-28T12:15:00Z
  ...
  test('timezone: every day at 08:00:00 in America/New_York', () => {
    const parser = new CronParser('0 0 8 * * ?', timeZoneNY);
    const fromNY = DateTime.fromISO('2025-09-28T12:00:00', { zone: timeZoneNY });
    const next = parser.next(fromNY);
    const prev = parser.previous(fromNY);
    const nextDate = DateTime.fromSeconds(next, { zone: 'UTC' }).setZone(timeZoneNY);
    const prevDate = DateTime.fromSeconds(prev, { zone: 'UTC' }).setZone(timeZoneNY);
  ...
  test('edge: invalid date (31st of February)', () => {
    expect(() => new CronParser('0 0 0 31 FEB ?', timeZone)).toThrow('Invalid date: day 31 is not valid for month February');
  });
  test('complex: every 2 hours on specific days and months', () => {
    const parser = new CronParser('0 0 */2 1,15 JAN,FEB ?', timeZone);
  ...
  test('edge: fifth Monday in short month', () => {
    const parser = new CronParser('0 0 0 ? FEB MON#5', timeZone);
