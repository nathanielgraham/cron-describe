...
export class CronParser {
    constructor(cronString, timezone) {
        this.seconds = { values: [0] };
        this.minutes = { values: [] };
        this.hours = { values: [] };
        this.daysOfMonth = { values: [] };
        this.months = { values: [] };
        this.daysOfWeek = { values: [] };
        this.years = { values: [] };
        this.dayOfMonthModifiers = {
            lastDay: false,
            lastDayOffset: 0,
            lastWeekday: false,
            nearestWeekday: false,
        };
        this.dayOfWeekModifiers = {
            nthDay: 0,
            lastDay: false,
        };
    ...
    parseField(field, min, max) {
        ...
        if (field.includes('/')) {
            ...
            if (step === 0)
                throw new CronError(`Invalid step value in ${field}`);
    ...
    parseDayOfMonth(field) {
        if (field === '?')
            return;
        let value = this.normalizeField(field, 'dayofmonth');
        if (value.endsWith('W')) {
    ...
    describeField({ values, step }, name, min, max, formatter) {
